...
export const partialCompile = (name: string, pqlVersion: string, data: QueryData, endSourceId: string, endOperatorId: string): Pql => {
  const endSourceIndex = data.sourceOrder
    .findIndex((source) => source === endSourceId);

  const sources = data.sourceOrder
    .filter((item, index) => index <= endSourceIndex)
    .map((sourceId) => {
      const source = data.sources[sourceId];
      let endOperatorIndex = source.operators
        .findIndex((operator) => operator === endOperatorId);
      ...
      if (endOperatorIndex === -1)
        endOperatorIndex = source.operators.length;
